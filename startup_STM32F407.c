 ...
 * @brief          : The start up file for the STM32F407xx microcontroller
						* Defines the vector table.
						* Implements the reset handler that performs operations
						  that needs to be done before invoking main()
 ******************************************************************************
 */

#include <stdint.h>

//The stack is placed at the end of SRAM.
...
// Exception and Reset Handler declarations
...
void Default_Handler(void){
	//Acts as a weak alias for all exception handlers
	while(1);
}